...
// 測試 WITHOUT ROWID 選項
TEST_F(SQLiteHelperTest, TableOptionWithoutRowId) {
    using IdColumn = Column<"id", column_type::INTEGER>;
...
// 測試 STRICT 選項
TEST_F(SQLiteHelperTest, TableOptionStrict) {
    using IdColumn = Column<"id", column_type::INTEGER>;
...
TEST_F(SQLiteHelperTest, TableOptionWithoutRowIdAndStrict) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    using EmailColumn = Column<"email", column_type::TEXT>;
...
TEST_F(SQLiteHelperTest, TableOptionWithoutRowIdCompositeKey) {
    using IdColumn = Column<"id", column_type::INTEGER>;
...
// 測試 STRICT 與多個約束組合
TEST_F(SQLiteHelperTest, TableOptionStrictWithConstraints) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    using EmailColumn = Column<"email", column_type::TEXT>;
    using PhoneColumn = Column<"phone", column_type::TEXT>;
...
// 測試基本批量插入
TEST_F(SQLiteHelperTest, InsertManyBasic) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    ...
    for (int i = 1; i <= 100; ++i) {
        rows.push_back(std::make_tuple(
            TestTable::MakeTableColumn<IdColumn>(i),
            TestTable::MakeTableColumn<NameColumn>("User" + std::to_string(i)),
            TestTable::MakeTableColumn<AgeColumn>(20 + (i % 50))
        ));
    }

    // 批量插入
    db.GetTable<TestTable>().InsertMany(rows);
    ...
    EXPECT_EQ(std::get<TestTable::TableColumn<IdColumn>>(results[0]).value, 1);
    EXPECT_EQ(std::get<TestTable::TableColumn<NameColumn>>(results[0]).value, "User1");
    EXPECT_EQ(std::get<TestTable::TableColumn<IdColumn>>(results[99]).value, 100);
    EXPECT_EQ(std::get<TestTable::TableColumn<NameColumn>>(results[99]).value, "User100");
}

// 測試批量插入空資料
TEST_F(SQLiteHelperTest, InsertManyEmpty) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    ...
    // 批量插入空資料，不應該拋出異常
    EXPECT_NO_THROW(db.GetTable<TestTable>().InsertMany(rows));
...
// 測試批量插入部分欄位
TEST_F(SQLiteHelperTest, InsertManyPartialColumns) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    ...
    EXPECT_EQ(std::get<TestTable::TableColumn<NameColumn>>(results[5]).value, "User6");
}

// 測試批量插入違反約束
TEST_F(SQLiteHelperTest, InsertManyWithConstraintViolation) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    using EmailColumn = Column<"email", column_type::TEXT>;
    ...
    // 應該拋出異常並回滾
    EXPECT_ANY_THROW(db.GetTable<TestTable>().InsertMany(rows));
...
// 測試批量插入與單筆插入混合使用
TEST_F(SQLiteHelperTest, InsertManyMixedWithSingleInsert) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    ...
    db.GetTable<TestTable>().Insert(
        TestTable::MakeTableColumn<IdColumn>(1),
        TestTable::MakeTableColumn<NameColumn>("Single")
    );
    ...
    for (int i = 2; i <= 11; ++i) {
        rows.push_back(std::make_tuple(
            TestTable::MakeTableColumn<IdColumn>(i),
            TestTable::MakeTableColumn<NameColumn>("Batch" + std::to_string(i))
        ));
    }
    ...
    // 再單筆插入
    db.GetTable<TestTable>().Insert(
        TestTable::MakeTableColumn<IdColumn>(12),
        TestTable::MakeTableColumn<NameColumn>("AnotherSingle")
    );
    ...
    EXPECT_EQ(std::get<TestTable::TableColumn<NameColumn>>(results[0]).value, "Single");
    EXPECT_EQ(std::get<TestTable::TableColumn<NameColumn>>(results[11]).value, "AnotherSingle");
}

// 測試批量插入效能（大量資料）
TEST_F(SQLiteHelperTest, InsertManyPerformance) {
    using IdColumn = Column<"id", column_type::INTEGER>;
    ...
    for (int i = 1; i <= 1000; ++i) {
        rows.push_back(std::make_tuple(
            TestTable::MakeTableColumn<IdColumn>(i),
            TestTable::MakeTableColumn<NameColumn>("User" + std::to_string(i)),
            TestTable::MakeTableColumn<AgeColumn>(20 + (i % 60)),
            TestTable::MakeTableColumn<ScoreColumn>(50.0 + (i % 50))
        ));
    }
