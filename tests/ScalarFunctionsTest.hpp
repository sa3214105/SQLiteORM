...
// 測試 TIME 函數
TEST_F(ScalarFunctionTest, TimeFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        Time("now"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();
    ...
    ASSERT_EQ(results.size(), 1);
    // 驗證返回的時間格式為 HH:MM:SS
    std::string timeStr = std::get<1>(results[0]);
    EXPECT_EQ(timeStr.length(), 8);
    EXPECT_EQ(timeStr[2], ':');
    EXPECT_EQ(timeStr[5], ':');
}

// 測試 DATETIME 函數
TEST_F(ScalarFunctionTest, DateTimeFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        DateTime("now"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();
    ...
    ASSERT_EQ(results.size(), 1);
    // 驗證返回的日期時間格式
    std::string datetimeStr = std::get<1>(results[0]);
    EXPECT_GT(datetimeStr.length(), 10);
}

// 測試 STRFTIME 函數
TEST_F(ScalarFunctionTest, StrftimeFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
    ...
    ASSERT_EQ(results.size(), 1);
    // 驗證返回的年份是4位數字
    std::string yearStr = std::get<1>(results[0]);
    EXPECT_EQ(yearStr.length(), 4);
}

// 測試 JULIANDAY 函數
TEST_F(ScalarFunctionTest, JulianDayFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        JulianDay("2023-01-01"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();

    ASSERT_EQ(results.size(), 1);
    // 2023-01-01 的 Julian Day
    double jd = std::get<1>(results[0]);
    EXPECT_NEAR(jd, 2459945.5, 1.0);
}

// 測試 UNIXEPOCH 函數
TEST_F(ScalarFunctionTest, UnixEpochFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        UnixEpoch("2023-01-01 00:00:00"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();

    ASSERT_EQ(results.size(), 1);
    // 2023-01-01 00:00:00 UTC 的 Unix timestamp
    int64_t timestamp = std::get<1>(results[0]);
    EXPECT_EQ(timestamp, 1672531200);
}

// ============ JSON 函數測試 ============

// 測試 JSON_ARRAY 函數
TEST_F(ScalarFunctionTest, JsonArrayFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        JsonArray(userTable[NameColumn], userTable[AgeColumn])
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();

    ASSERT_EQ(results.size(), 1);
    std::string jsonStr = std::get<1>(results[0]);
    EXPECT_TRUE(jsonStr.find("Alice") != std::string::npos);
    EXPECT_TRUE(jsonStr.find("25") != std::string::npos);
}

// 測試 JSON_OBJECT 函數
TEST_F(ScalarFunctionTest, JsonObjectFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        JsonObject("name"_expr, userTable[NameColumn], "age"_expr, userTable[AgeColumn])
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();
...
// 測試 JSON_EXTRACT 函數
TEST_F(ScalarFunctionTest, JsonExtractFunction) {
    auto jsonStr = "{\"name\":\"Alice\",\"age\":25}"_expr;
    auto results = userTable.Select(
        userTable[NameColumn],
        JsonExtract(jsonStr, "$.name"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();
...
// 測試 JSON_VALID 函數
TEST_F(ScalarFunctionTest, JsonValidFunction) {
    auto results = userTable.Select(
        userTable[NameColumn],
        JsonValid("{\"name\":\"Alice\"}"_expr),
        JsonValid("invalid json"_expr)
    ).Where(userTable[NameColumn] == "Alice"_expr).Results().ToVector();

    ASSERT_EQ(results.size(), 1);
    EXPECT_EQ(std::get<1>(results[0]), 1);  // Valid JSON
    EXPECT_EQ(std::get<2>(results[0]), 0);  // Invalid JSON
}
